...
| `CARDS_PASSED` | `{ revealed: { fromUserId, toUserId, card }[] }` | 패스 카드 동시 공개 후 |
| `TURN_CHANGED` | `{ currentTurnUserId, expiresAt }` | 턴 전환 |
...
| type | payload | 의미 |
| --- | --- | --- |
| `HAND_DEALT` | `{ cards[8] }` | 첫 8장. Grand 선언 가능. |
| `HAND_DEALT_FULL` | `{ cards[14] }` | 나머지 6장 추가. Tichu 선언 가능. |
...
| type | payload | 비고 |
| --- | --- | --- |
| `DECLARE_GRAND_TICHU` | `{}` | 첫 8장 단계에서만 |
| `DECLARE_TICHU` | `{}` | 본인 첫 카드를 내기 전 |
| `PASS_CARDS` | `{ toLeft: cardRef, toPartner: cardRef, toRight: cardRef }` | 14장 받은 후 1회 |
| `PLAY_CARD` | `{ cards: cardRef[], phoenixAs?: cardRef }` | 일반 플레이 |
| `PASS` | `{}` | 트릭 패스 |
| `MAKE_WISH` | `{ rank: 2..14 }` | Mahjong을 낸 직후만 |
| `GIVE_DRAGON_TRICK` | `{ toUserId }` | Dragon으로 트릭 가져간 직후 |
| `BOMB` | `{ cards: cardRef[] }` | 별도 핸들링이 필요한 경우 (그렇지 않으면 PLAY_CARD 로 통합) |
