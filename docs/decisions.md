...
## D-103 (2026-07-31) — 스컬킹 클라: RoomEventSink 주입 + Row-Flow 가변 좌석 (M5/T7 S6)

**클라의 방 소켓 계층을 게임 중립으로 만든다.** `useStompRoom` 이 `useTichuStore` 를
하드코딩하고 있어(import + 셀렉터 6개 + 비공개 큐 3분기) 스컬킹 이벤트를 받을 자리가 없었다.
훅의 이름·위치·반환 5필드를 그대로 두고 **입력만** `RoomEventSink` 주입으로 확장한다 —
훅은 게임 스토어를 모르고, 게임별 sink 파일이 스토어에 꽂는다. 비공개 큐는 `ERROR` 까지
전량 sink 로 위임한다(같은 본인 큐 프레임인데 `HAND_DEALT` 는 게임이 `ERROR` 는 훅이 처리하는
비대칭 제거 + 스컬킹 고유 코드 10종을 한국어 라벨로 바꿀 자리 확보). 훅에 남는 `setError` 는
REST resync 실패 전용으로 의미가 좁아진다. `tichuStore`(539줄)는 **0줄 수정**(D-95 이월 존중).

sink 규약 두 가지가 안전의 핵이다: **모듈 상수**여야 하고(렌더마다 새 객체 금지), 각 메서드는
**호출 시점에 `getState()`** 를 읽어야 한다. 훅이 `sinkRef` 로 sink 를 deps 에서 빼기 때문에
(인라인 객체를 넘겨도 소켓 재연결·resync 루프가 원리적으로 불가능) 그 대가로 stale closure
위험이 생긴다. 대안(`useRoomSocket` 신규 추출 + 티츄 어댑터)을 버린 이유는 옮기는 코드량이다 —
추출안은 티츄가 실제 실행하는 201줄을 이식하고, 주입안은 import 2줄·셀렉터 6→1줄·분기 이동으로
끝난다. 기존 `GameTable.test.tsx` 가 `useStompRoom` 을 인자 무시 팩토리로 모킹하므로 이 수술은
144건이 검증해주지 못한다 — **옮긴 줄 수가 곧 미검증 위험량**이라 적은 쪽이 옳고, 부족한
안전망은 신규 테스트 2종으로 메운다.

**2~8 가변 좌석은 절대배치 원형이 아니라 Row-Flow 로 간다.** 좌석은
`repeat(auto-fit, minmax(...))` 그리드라 **폭 미디어 쿼리 0개로** 상대 1~7명이 자동 줄바꿈되고
겹침이 레이아웃 엔진 차원에서 불가능하다. 극좌표 링은 ≤440px 8인에서 클리어런스가 20~49px 이고
jsdom 이 그걸 못 잡아 실기기 수동 튜닝 루프가 된다 — 실패 모드가 "겹쳐서 못 읽음"(닫힌·조용한)
대 "세로 스크롤"(열린·sticky 방어 가능)로 갈린다. D-76 이 좁은 화면 2×2 grid 를 폐기한 이유는
티츄가 2:2 팀전이라 파트너/상대 **방위축**을 보존해야 했기 때문인데, 스컬킹은 개인전이라
보존할 방위축이 없다. 대신 보존해야 하는 정보는 **재생 순서**다(리드 수트가 첫 색상 카드로
지연 확정되고 비추이적 순환이 있어 "누가 먼저 냈나"가 판정 근거) — 트릭을 중앙 단일 패널이
아니라 재생순 레일로 늘어놓고 이름칩·순번 배지로 잇는다. 8인 45° 링에서는 카드가 두 좌석
사이로 보여 "카드 위치 ↔ 낸 사람" 대응 자체가 모호해지므로 원형의 이점이 정작 8인에서 사라진다.
CSS 는 신규 `18-skullking-table.css` 에 `.sk-` 접두로 격리하고 16 뒤·**17 앞**에 import 한다
(17-responsive 가 마지막이라는 D-88 불변식 유지). 티츄 기하(4치 방위·fan-*·중앙 단일 트릭
패널)는 재사용 불가 판정.

**라운드 경계 resync 는 설계로 수용한다.** 비공개 `HAND_DEALT` 가 좌석 수만큼 seq 를 태우고
클라 본인 큐 핸들러는 `lastSeq` 를 전진시키지 않으므로, 뒤따르는 `BIDDING_STARTED` 는 거의
항상 gap 이다 — 리듀서를 붙여도 seq 게이트가 먼저 걸려 무의미하다. 단 지난 라운드의 공개된
예측값이 화면에 남는 건 혼동이므로 `BIDDING_STARTED` 는 **seq 판정과 무관하게** 라운드 로컬
상태를 즉시 스크럽한다. 스컬킹 방에는 칩/판돈 UI 가 없다(`RoomChipService` 는 티츄 게이트).
**부채 명기**: `tichuStore` 동결 때문에 seq 판정 약 40줄이 두 스토어에 중복된다 — 3번째 게임
전에 "판정은 훅으로, sink 는 순수 리듀서로" 승격이 필요하다.

**C5 실측(375×667, 8인 봇 방)이 잡은 결함 4건.** 자동 테스트가 원리적으로 못 잡는 것들이라
실기기 확인을 완료 조건으로 둔 것이 실제로 값을 했다. ① `minmax(100px, 168px)` 처럼 max 가
**고정값**이면 CSS 스펙상 `auto-fit` 반복 횟수를 max 기준으로 세므로 375px 화면에서 트랙이
1개만 생겨 8인이 세로로 쌓였다 — max 를 `1fr` 로 바꿔야 min 기준(3열)으로 센다. ② 게임판은
`.app-shell` 밖이라 tailwind 의 `border-box` 리셋이 닿지 않아 좌석이 트랙을 21px 넘쳤다 —
스코프 안에서 명시했다. ③ 105px 좌석에서 `예측/획득/손패` 라벨이 2줄로 쪼개져 한 글자
(`예/획/손`, 뜻은 `title`)로 줄였다. ④ 비활성 제출 버튼이 반투명 흰색이라 **라이트 테마에서
흰 배경 위 흰 글자**로 읽히지 않았다 — 불투명 회색으로 고정했다. 추가로 라운드 경계 과도기에
지난 라운드 손패가 남아 클릭 가능한 문제를 발견해 `BIDDING_STARTED` 스크럽이 손패를 비우고
좌석 손패 수를 payload 의 `handSize` 로 즉시 맞추게 했다.

**티츄 회귀 없음(라이브 확인).** 4좌석 고정 기하·팀 링(청/적)·아바타 동물 폴백·손패 8장·
그랜드티츄 선언 UI 가 그대로였다 — C1 의 비공개 큐 위임(`HAND_DEALT` → sink)과 C3 의 글리프
...
노출된다. 셋 다 티츄 전용이다 — 스컬킹은 10라운드 고정이라 목표 점수가 무의미하고, 개인전이라
팀이 없고, 칩 정산은 `RoomChipService` 가 `TichuGameDefinition.ID` 로 게이팅한다. **판돈은
무해하지 않다**: 스컬킹 방에서 켜면 `RoomChipService` 가 조용히 return 해서 칩은 생기지
않는데, `stake > 0` 이라 봇 채우기만 금지된다. UI 는 "승팀 +N칩"을 약속하고 아무 일도
일어나지 않는다.

**선택.** 게임이 자기가 쓰는 방 옵션을 선언한다 — `GameDefinition.supportedRoomOptions()`
→ `Set<RoomOption>`(`TARGET_SCORE`·`TEAMS`·`BETTING`). 기본값은 **빈 집합(옵트인)**이라
새 게임은 한 줄도 안 써도 안 쓰는 옵션이 안 뜬다. D-102 의 "새 게임 = 패키지 + Bean" 약속을
깨지 않으려는 것이다. 티츄만 세 옵션을 명시적으로 선언한다.

**배제 논거.** 클라에서 `gameId` 로 분기하면 게임을 추가할 때마다 클라를 고쳐야 해서
D-103(클라 인게임 게임 중립)의 취지와 어긋난다. 포트에 boolean 3개를 다는 방식은 옵션이
늘 때마다 포트 메서드가 늘어난다 — D-97/D-98 이 좁게 유지하려던 바로 그 표면이다. `Set` 은
표면을 1개로 고정하고 옵션 추가를 enum 상수 1개로 만든다.

**서버도 거절한다.** 클라가 안 보내게 고쳐도 서버는 검증해야 한다(Server-Authoritative —
클라 입력은 검증 대상이지 신뢰 대상이 아니다). 미지원 옵션에 **기본값이 아닌 값**이 오면
`UNSUPPORTED_ROOM_OPTION` 으로 거절한다. 기본값 카브아웃은 기존 호출부·테스트 호환용이고,
"조용히 무시"를 택하지 않은 이유는 그 조용함이 애초에 이 결함을 만들었기 때문이다.

**정정 (같은 날) — `TEAMS` 는 노출 게이트가 아니라 라벨 결정자다.** 처음엔 팀 배정 UI 를
`TEAMS` 없는 게임에서 **숨기고** 서버도 거절했는데, 이건 틀렸다. 실측: `domain.game` 전체에
`TeamPolicy` 참조가 **0건**이고, `RANDOM` 이 하는 일은 `RoomService.onGameStart` 의
`Collections.shuffle(playerIds)` + `replacePlayerOrder` 하나뿐 — **게임 중립적인 좌석 순서
셔플**이다. 스컬킹은 좌석 순서가 곧 턴 순서라 섞는 것이 의미 있다. 즉 동작하는 기능을
없애고 정당한 요청을 400 으로 막은 셈이다.

근거는 이 결정 자신이 이미 갖고 있었다 — "모든 게임에 통하는 설정은 여기 넣지 말 것".
좌석 정책은 인원·턴 제한·봇 채우기와 같은 부류인데 `TEAMS` 로 묶어버린 것이 실수였다.
`TEAMS` 가 정말 결정하는 것은 **라벨**뿐이다: 팀전이면 "팀 배정", 개인전이면 "좌석 순서".
따라서 `teamPolicy` 거절을 생성·변경 두 경로에서 모두 걷어내고, 클라는 행을 숨기는 대신
라벨만 바꾼다. `TARGET_SCORE`(10R 고정이라 무의미)·`BETTING`(칩 정산이 팀 승패에 묶임)
게이팅은 그대로 유효하다.

발견 경위를 남긴다: 같은 날 병렬로 돌던 다른 세션이 이 지점을 정확히 짚었고, 그 주장을
`grep`·코드 확인으로 검증해 채택했다. 브라우저 확인에서 "스컬킹 + 팀RANDOM → 400"을
**예상대로**라고 보고했었는데, 예상 자체가 틀렸던 것이다 — 통과하는 걸 보고도 기대값이
틀렸을 가능성을 의심해야 한다는 사례.

**대가.** 계약이 넓어진다 — `GET /api/games` 와 방 응답 양쪽에 `supportedRoomOptions` 가
붙는다. 방 응답에도 실은 것은 대기실이 팀 배정 노출 여부를 알려고 게임을 따로 부르면
요청 워터폴이 생겨 배치가 한 박자 늦게 흔들리기 때문이다(정적 메타의 소폭 중복을 감수).

## D-105 (2026-07-31) — 쇼케이스 마감: README 재편 + 데모 계정 게이팅 + CD (M4/트랙 G)

**README 를 개발자 런북에서 포트폴리오 진입점으로 재편한다.** 현 615줄은 Phase 2b~2e 동작
확인 절차·로컬 부트스트랩·Phase 6 시연 체크리스트가 대부분이라, 처음 온 사람이 "이게
무엇이고 왜 볼 만한가"를 알 수 없다. 런북 내용은 버리지 않고 `docs/` 로 옮긴다 — 실제로
쓰이는 절차이고, 옮기면 README 가 짧아지는 동시에 문서가 목적별로 정리된다.

**데모 계정은 Flyway 마이그레이션이 아니라 환경변수 게이트 시더로 만든다.** 마이그레이션에
넣으면 공개 비밀번호를 가진 계정이 **모든 환경**(로컬·CI·향후 어떤 배포든)에 자동으로
생긴다 — 되돌리려면 또 마이그레이션을 써야 한다. `mirboard.demo.enabled` 기본 false 로 두고
배포 환경에서만 켜면, 켜고 끄는 것이 설정 한 줄이고 CI 는 영향을 받지 않는다. 비밀번호
해시는 런타임에 기존 `PasswordEncoder` 로 만들므로 해시를 소스에 박지 않아도 된다.

**CD 는 워크플로만 준비하고 실제 배포는 사용자가 실행한다.** `FLY_API_TOKEN` 은 계정
소유자만 발급할 수 있고, 배포는 되돌리기 어려운 외부 행위다. `.github/workflows/deploy.yml`
을 수동 트리거(`workflow_dispatch`) + main 푸시로 두되, 시크릿이 없으면 아무 일도 일어나지
않는다.

**데모 GIF 대신 정적 스크린샷을 쓴다.** 이 머신에 `ffmpeg`·`imagemagick`·`gifski` 가 없어
GIF 를 만들 수 없다. 다만 정적 이미지가 README 에서 더 나은 선택이기도 하다 — GitHub 에서
즉시 렌더되고, 로딩이 가볍고, 독자가 자기 속도로 본다. 필요하면 나중에 GIF 를 추가한다.

**케이스 스터디는 "성공담"이 아니라 "추상이 샌 지점"을 중심에 둔다**
(`docs/case-study-multi-game.md`). 두 번째 게임이 포트 계약의 결함을 정확히 한 곳
(`boolean desert(...)` 2치 → 3치) 찾아냈다는 사실이, "처음부터 완벽했다"보다 강한 설계
증거다. 각 절을 `문제/후보/선택/배제 논거/대가` 로 쪼개고 `코드:/테스트:/결정:` 앵커를
달아, 한 절만 읽은 사람도 그 자리에서 검증할 수 있게 했다. 부록의 재현 명령은 전부 실행해
본문 수치와 대조했다 — 재현 명령을 싣는 문서에서 본문 숫자가 출력과 어긋나면 절 전체가
...
무변경, 인프라 변경은 포트 계약 확장 1건"이라는 정확한 서술로 바꿨고, 이쪽이 검증 가능해서
더 강한 주장이다. 나머지 셋은 로드맵 M5 미반영, `HAND_DEALT_FULL` 죽은 참조, Gradle 버전
드리프트(문서 8.10.2 / 실제 wrapper 9.4.1).

## D-104 (2026-07-31) — 스컬킹 개인전 탈주: 유령 좌석 자동조종으로 "남은 사람끼리 계속"

티츄의 탈주 처리(D-75, 상대팀 승리로 매치 즉시 종료)는 2:2 팀전 전제라 개인전에 옮길 수
없어, 스컬킹은 **남은 사람끼리 계속**으로 확정한다(사용자 결정). 메커니즘은 **유령 좌석**:
탈주 좌석을 제거하지 않고 `SkullKingMatchState.desertedSeats` 표식만 남긴 뒤, 그 좌석의
차례를 엔진이 자동조종(미제출 예측 0 고정 — 이미 제출한 예측은 불변, 카드는 최약수 전순서,
티그리스는 탈출 선언)으로 대신 둔다. 자동조종은 `timeoutAction` 과 같은 정책을 공유한다.

좌석 제거(즉시 이탈·라운드 경계 축소)를 배제한 이유: `seatCount` 가 트릭 크기
(`played.size() >= seatCount`)·턴 모듈러 산술·카드 보존(`handSize × seatCount`)·
좌석==인덱스 전제에 동시에 묶여 있어, 좌석을 빼는 순간 조용한 오판정 표면이 코드 전역에
생긴다. 유령은 정상 `apply` 경로로만 카드를 옮기므로 판정 로직·불변식 체커·기존 테스트가
전부 무변경이고, 탈주 경로 전체가 Docker 없는 순수 시뮬레이션으로 검증된다.

조기 종료는 두 조건뿐이다 — 잔존 좌석 < 2(§2 최소 인원) 또는 잔존 **사람** 0(봇만 남은
매치는 무의미). 이때 진행 중 라운드는 점수 미반영으로 폐기하고 `roundNumber` 를 11 로
점프시켜(`abandoned()`) 포트의 무인자 `isMatchOver()` 계약을 재사용한다. 탈주 좌석은 최종
승자 후보에서 제외하되 점수 궤적은 계속 기록·공개한다(라운드별 누적 표 정합). 멱등 가드는
`isMatchOver ∥ 이미 탈주 ∥ 매치 참가자 아님` 3연, 탈주 좌석의 사람 액션은 신규
`SEAT_DESERTED` 로 거절한다.

사용 계약이 하나 늘었다: 사람 액션 뒤와 라운드 시작 뒤에 유령 차례를 드레인해야 한다 —
`applyAndDrain`/`startRoundAndDrain` 이 이를 봉인하고, 2-인자 불변식 체커가 "탈주 좌석이
대기 좌석에 남아 있지 않다"로 드레인 누락을 검출한다. **한계(별건)**: 끊김 유예(120s) 구간은
탈주 확정 전이라 자동조종이 켜지지 않는다 — S5 에서 결정. 룰 확정 3건은
`rules-skullking.md` §13-⑱⑲⑳. (번호 주의: D-102·D-103 은 S5·S6 예약.)

## D-101 (2026-07-31) — 스컬킹 도메인: 룰 구현 (M5/T7 S4, 신규 패키지)

`domain/game/skullking/` 신설. D-100 룰 명세를 코드로 옮긴다. 선행 S1(D-98 포트)·
S3(D-100 명세) 완료 확인 후 착수. 티츄 룰은 손대지 않는다 — 신규 패키지라 파일 교집합이
0이므로 회귀 원인이 섞이지 않는다.

**S4 경계를 "순수 룰 엔진까지"로 확정한다.** 계획서는 항목 5를 "`SkullKingEngine` 포트
구현"이라고 적었지만, `GameEngine` 포트는 `loadState`/`saveState`/`actionType`/`advance`/
`desert` 를 요구해 Redis 스토어와 Spring 이 딸려 온다. 그런데 S5 범위에 "상태 저장/뷰
매퍼"가 이미 명시돼 있어 겹친다. 티츄가 순수 `TichuEngine`(539줄) + 어댑터
`TichuGameEngine`(323줄)로 이미 이 문제를 풀었고 `GameEngine` javadoc 도 그 구조를
권장하므로 같은 선을 긋는다 — S4 는 순수 엔진까지, 어댑터·`GameDefinition` 등록·스토어는
S5. 결과적으로 S4 산출물은 Docker 없이 순수 단위 테스트로 전량 검증된다.

**티그리스 정체성을 제출 시점에 1회 해소한다.** 명세 함정 #10 은 티그리스가 강약·동점·
보너스 **세 군데 모두**에서 선언값으로 취급돼야 하고 한 군데만 빠뜨리면 조용히 어긋난다고
경고한다. 세 곳이 각자 `if (티그리스 && 해적선언)` 을 쓰면 반드시 어긋나므로,
`PlayedCard.kind()` 가 `SUIT|ESCAPE|PIRATE|MERMAID|SKULL_KING` 로 해소한 값을 단일
출처로 두고 그 뒤로는 아무도 티그리스를 모르게 한다.

**리드 수트는 저장하지 않고 제출 순서에서 파생한다.** 명세 §6.1 의 지연 확정 때문에 리드
수트는 트릭 도중에 확정될 수도, 끝까지 미확정일 수도 있다. 필드로 들고 있으면 카드를 낼
때마다 갱신해야 하고 한 경로만 빠뜨려도 stale 해진다. 트릭당 카드가 최대 8장이라 매번
파생해도 비용이 없다.

**Dealing 을 상태로 만들지 않는다.** 명세 §3 의 Dealing 은 플레이어 액션이 0개인 통과
지점이라(티츄의 Dealing 은 그랜드티츄 선언 윈도우였다) 상태로 두면 아무도 살지 않는 분기가
생긴다. `Bidding → Playing → RoundEnd` 3단계로 간다.

**불변식은 70이 아니라 `handSize × seatCount` 로 잡는다.** 8인 라운드 9·10 은 70장 중
6장을 쓰지 않으므로(§4) 덱 전체를 기준으로 하면 그 두 라운드에서 거짓 위반이 난다. 중복
검사도 Set 이 아니라 multiset 이다 — 덱에 같은 해적이 5장 있어 Set 크기로는 보존을 확인할
수 없다. 대신 종류별 출현 횟수가 덱 허용치를 넘는지 본다. **중복 특수 카드에 copy 인덱스를
두지 않기로** 한 결과인데, 약해 보이지만 실제로는 그렇지 않다: 색상 카드는 덱에 1장뿐이라
이 검사만으로 색상 카드 복제가 잡히고, 특수 카드끼리의 뒤바뀜은 애초에 게임상 구별 불가능한
상태라 위반이 아니다. copy 인덱스를 달았다면 클라가 "몇 번째 해적을 내는지"까지 왕복시켜야
하고, 같은 값의 다른 인덱스를 보냈다는 이유로 정당한 플레이가 거절되는 함정이 생긴다.

**결과.** 테스트 275건 전량 그린, 서버 전체 687건 실패 0 (티츄 회귀 없음). 스컬킹 275건은
Spring·Redis·Docker 를 전혀 쓰지 않아 `./scripts/check.sh rules` 에 편입했다(~5s).
2~8인 **전 좌석 수**가 무작위 합법수만으로 10라운드를 완주하며, 매 액션 직후 불변식
체커를 통과한다. 명세가 지정한 판별 테스트 두 개도 그대로 고정했다 — `라운드 3, bid=0,
won=1 → −30`(일반칙 해석이면 −10)과 3자 트릭 인어 승 `40점`(포함 기준이면 90점). 불변식
체커는 통과 케이스뿐 아니라 고의로 깨뜨린 상태 8건(+오탐 방지 통과 2건)으로 실제 검출까지
검증했다 — 안 터지는 체커는 아무것도 보장하지 않기 때문이다.

**S5 로 넘긴 것 중 룰 결정이 남은 하나: 개인전의 탈주 처리.** 티츄의 "상대팀 승리"(D-75)는
2:2 라 성립하는 규칙이라 개인전에 그대로 옮길 수 없다. 남은 사람끼리 계속할지 매치를
무효로 할지는 S5 착수 전에 별도 D 항목으로 정한다.

## D-100 (2026-07-30) — 스컬킹 룰 명세 (M5/T7 S3, 코드 변경 0)

`docs/rules-skullking.md` 신설. D-56(`rules-tichu.md`)이 "룰 ↔ 코드 1:1 매핑의 단일
진실원"으로 잘 작동한 선례를 그대로 따른다 — 구현 중 룰 해석을 놓고 헤매는 비용이 문서를
먼저 쓰는 비용보다 크다. 코드는 S4(D-101)에서. **번호 순서 주의**: D-98(S1 포트 추출)은
아직 미착수이고 D-99(S2 인원 가변)가 먼저 진행됐다 — S3 는 문서 전용이라 파일 교집합이
`decisions.md` 뿐이어서 예약 번호대로 병행했다.

**핵심 설계 판단 — 비추이적 3자 순환을 5단 우선순위 사다리로 환원한다.** 해적>인어,
스컬킹>해적, 인어>스컬킹 은 대소 비교로 표현할 수 없고, 여기에 "셋이 다 나오면 인어 승"
이라는 명시 예외가 얹힌다. 그런데 예외를 포함한 7가지 조합 전체가 `스컬킹 && 인어 → 인어`
/ `스컬킹 → 스컬킹` / `해적 → 해적` / `인어 → 인어` / `색상` / `탈출` 의 순차 사다리와
정확히 일치한다. 조건 분기를 늘리지 말고 이 사다리를 테이블로 구현한다.

**원문 미규정 2건을 해석으로 확정했다.** ① 리드 수트: "탈출로 리드하면 다음 색상 카드가
리드 수트"와 "캐릭터 카드로 리드하면 리드 수트 없음"만 명시돼 있어, *탈출 리드 → 캐릭터
카드 → 색상 카드* 순서가 미규정이다. **트릭의 첫 카드가 캐릭터면 그 트릭엔 리드 수트가
없고, 그 외에는 처음 나온 색상 카드의 색이 리드 수트**로 정한다. 이 선택은 승자 판정을
바꾸지 않는다(캐릭터 카드가 트릭에 있으면 색상 카드는 어차피 못 이긴다) — 영향 범위가
후속 플레이어의 follow 의무뿐이라 오판정 위험이 없다. ② 스컬킹 포획 보너스에서 해적 선언
티그리스를 해적 1장으로 센다(강약 판정에서 이미 해적으로 취급하므로 일관).

**구현 함정 3건을 문서에 못 박았다.** ① 8인 라운드 9·10 은 70장으로 9·10장씩 못 나누므로
`min(라운드, ⌊70/N⌋)` = 8장 — 트릭 수는 실제 배분 장수지만 0승 보너스는 여전히 *라운드
번호* × 10 이다. ② 0승 예측 실패는 차이가 아니라 정액 라운드 번호 × 10 감점. ③ 포획
보너스가 "무엇으로 이겼는가"에 달려 있어 승수 카운터로는 계산이 불가능하다 — 트릭별로
(승리 카드, 포함 카드 전체)를 보관해야 한다.

**원문 전수 대조로 미규정이 2건 → 17건으로 늘었다.** 5개 도메인(덱/입찰/색상판정/특수판정/
점수)을 각각 추출한 뒤 도메인마다 "원문이 답하지 않는 도달가능 케이스"를 적대적으로 사냥했다.
그 결과 착수 시 잡았던 2건 외에 15건이 더 나왔고, 전부 `rules-skullking.md` §13 에 근거와
함께 확정해 뒀다. 그중 셋은 성격이 다르다.

① **각주와 본문이 정면 충돌한다.** 각주 [4]는 "리드 수트와 다른 색의 색상 카드는 숫자가
아무리 커도 반드시 패배"라고 무조건으로 쓰는데, 본문은 "검정(으뜸패)은 다른 3색을 이긴다"고
한다. 검정 3 vs 초록 11(리드 초록)에서 정반대 승자를 지시하고 우선순위가 없다. 본문을
우선하고 각주는 검정을 뺀 3색에만 적용되는 부연으로 읽는다 — 각주를 그대로 옮기면 으뜸패가
죽는다.

② **"잡았다"의 정의가 없다.** 보너스 성립 조건은 "트릭에서 얻은 카드 중 포함되어 있다면"
이라는 단순 포함인데, 항목은 "해적으로 잡은 인어"처럼 관계형이다. 인어·스컬킹·해적이 다 나온
트릭을 인어가 이기면 포함 기준 90점 / 관계 기준 40점으로 갈린다. 관계 기준을 택한다.

③ **0 예측 실패에 두 조항이 동시에 적용된다.** 일반 실패(차이×10)와 0 예측 특칙(라운드×10)이
둘 다 문면상 이 상황을 덮는데 우선순위 문장이 없다. 더 좁은 조건인 특칙이 대체하는 것으로
정하고 합산은 배제한다. 두 해석을 구분하려면 `라운드 3, bid=0, won=1`(-10 vs -30)을 회귀로
고정해야 한다 — `라운드 10, bid=0, won=10` 은 둘 다 -100 이라 회귀를 못 잡는다.

**티그리스의 선언값이 세 군데에 걸린다.** 강약 판정·동점 판정("먼저 낸 해적")·포획 보너스
모두에서 선언한 카드로 취급해야 한다. 원문이 티그리스를 해적 범주로 묶은 문장은 스컬킹 강약
설명 하나뿐이라, 나머지 두 곳으로의 전이는 우리 해석이다. 한 군데만 빠뜨리면 조용히 어긋난다.

**8인 예외의 파급이 셋으로 갈린다.** 8인 라운드 9·10 은 70장으로 9·10장씩 못 나눠 8장이
되는데, 여기서 **트릭 수와 예측 상한은 손패(8)**, **0 예측 점수만 라운드 번호(9·10)** 를
따른다. 그 결과 손패가 같은 두 라운드의 0 예측 배점이 90/100 으로 갈리는 어색함이 남지만,
"이번 라운드 수"를 라운드 번호로 읽는 것이 문면에 가깝다(같은 절이 "제 1 라운드"처럼 번호로
지칭하고, "트릭 수"는 용어 절에 따로 정의된 용어인데 여기 쓰이지 않았다).

**상급자 카드 4종(약탈품 2·크라켄·흰고래)은 범위 밖.** 크라켄은 "승자 없음"이라는 새 결과
종류를, 흰고래는 판정 전 카드 의미 재작성을, 약탈품은 플레이어 간 점수 동맹을 도입해 셋 다
위 사다리 자체를 흔든다. 기본 룰 안정화 후 별건. 출처가 나무위키(집단 편집, 2021년판에서
최대 인원 6→8 변경)라 퍼블리셔 규칙서와 어긋날 수 있음을 문서 머리에 명시했다.
