...
> 상태: **T0·T1·T3·T4·T6 완료, T5 1차 완료(D-89/90/92/93/94/96), T2·T7·T8 대기** — 착수는 Phase Gate 승인 후
...
| 트랙 | 로드맵 | 병렬 | 충돌 대상 |
| --- | --- | --- | --- |
| ~~**T0** 계약 문서 기준선 정정~~ | (선행) | ❌ 단독 | ✅ **완료 (D-89, 2026-07-30)** |
| ~~**T1** 엣지 하드닝~~ | M2-C1 레이트리밋 | ✅ | ✅ **완료 (D-90, 2026-07-30)** |
| **T2** 관측 스택 | M2-C3 Sentry+Grafana | ✅ | T1·T3·T5·T6 |
| ~~**T3** 운영 런북·부하~~ | M2-C5 백업+k6 | ✅ | ✅ **완료 (D-92, 2026-07-30)** |
| ~~**T4** 채팅 신고 수직 슬라이스~~ | M2 D-86 후속 | ✅ | ✅ **완료 (D-93, 2026-07-30)** |
| 🔶 **T5** 클라 UX 부채 | styles.css/tichuStore 분해 + i18n | ✅ | **1차 완료 (D-94)** / 2차·store·i18n 이월 |
| ~~**T7** 포트 설계~~ | M5 선행 설계 | ✅ | ✅ **완료 (D-97, 2026-07-30)** → `multi-game-sessions.md` 로 분기 |
| ~~**T6** 수평확장~~ | M3 | ❌ 단독 | ✅ **완료 (D-96, 2026-07-30)** |
...
아래가 착수 전 진단이었고, 2026-07-30 전수 대조로 정정을 마쳤다. 엔드포인트 24종·
STOMP 이벤트 28종 양방향 기계 대조에서 불일치 0건. 이제 나머지 트랙이 같은 기준선
위에서 병렬 진행 가능하다.
...
3. **Flyway 신규 마이그레이션 + JPA 엔티티** — 버전 번호 단일 네임스페이스, `ddl-auto:
   validate` 라 엔티티와 같은 커밋 강제. → 번호 사전 배정(**V9 소진(D-93) → 다음 V10**).
4. **계약 변경은 서버 DTO + 클라 미러 + docs 를 한 커밋에** — OpenAPI/코드생성기가
   ...
   불일치 상태.)
5. ~~**`styles.css` 편집**~~ → **D-94 로 17파일 분할 완료** — 컴포넌트별 편집면이 갈려
   CSS 는 더 이상 단일 병목이 아니다. 단 `styles/parts/17-responsive.css`(폭 미디어 전량)와
   `styles/index.css`(@import 순서)는 여전히 **공유 파일**이라 직렬화 대상.
   **`tichuStore.ts` 는 미분리** → 클라 스토어 작업은 여전히 동시 1트랙.
...
1. 트랙 착수 전 `decisions.md` 에 **D 번호 사전 예약 + 제목만 있는 스텁을 main 에 먼저
   커밋**. D-89 까지 소진(D-87 GameTable 분해 / D-88 UI 보정 / D-89 T0) →
   D-90·D-92·D-93·D-94·D-96·D-97 소진 → 다음 배정: **T2=D-91, T5 2차=D-95**,
   멀티게임 세션 **S1=D-98 · S2=D-99 · S3=D-100 · S4=D-101 · S5=D-102 · S6=D-103**
   (`docs/plans/multi-game-sessions.md`). 본문은 트랙 브랜치에서 채운다.
