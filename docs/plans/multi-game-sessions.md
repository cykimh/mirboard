...
| ~~S5~~ | 스컬킹 통합 (엔진·봇·디스패치) | S2·S4 | D-102 | 서버 | ✅ **완료** — 봇 풀매치 IT |
| ~~S6~~ | 스컬킹 클라 | S5 | D-103 | 클라 | ✅ **완료** — Row-Flow 기하, 실측 완료 |

**병렬 가능 조합**: S4 는 신규 패키지라 병행 가능.
...
## ~~S3 — 스컬킹 룰 명세 (문서 전용)~~ — ✅ 완료 (D-100)

**D-100**. 코드 변경 0.

### 결과
**산출물 `docs/rules-skullking.md`(469줄)** — S4/S5 의 룰 정본이다. 아래 "반드시 담을 것"과
"설계 힌트"는 그 문서에 반영됐고, 구현 시에는 이 블록이 아니라 **`rules-skullking.md` 를
정본으로 본다**.

특히 넘겨받을 세 가지(자세한 근거는 D-100):
- **비추이적 3자 순환은 5단 우선순위 사다리로 환원된다** — 예외 포함 7가지 조합 전체가
  `스컬킹+인어 → 인어` / `스컬킹` / `해적` / `인어` / `색상` / `탈출` 순차 사다리와 일치한다.
  조건 분기를 늘리지 말고 테이블로 구현할 것.
- **원문 미규정 17건을 §13 에 근거와 함께 확정**했다(착수 시 2건 → 전수 대조로 15건 추가).
- **구현 함정 3건**: 8인 라운드 9·10 은 70장 한계로 `min(라운드, ⌊70/N⌋)`장 배분(트릭 수는
  실제 배분 장수지만 0승 보너스는 여전히 *라운드 번호* × 10) · 0승 예측 실패는 차이가 아니라
  정액 감점 · 포획 보너스는 승수 카운터로 계산 불가라 트릭별 (승리 카드, 포함 카드)를 보관해야 한다.

<details>
<summary>착수 시 지시(기록용)</summary>
